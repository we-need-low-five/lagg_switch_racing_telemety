...
- ACC lap boundaries are driven by the `normalized_car_position` start/finish wrap, **not** solely `completed_lap` (which stalls across out-laps / return-to-garage). Out / in-laps are recorded tagged **invalid**. Lap time is `graphics.last_time` (`iLastTime`) only when it is in a sane band **and** agrees with `max_current_time_ms` (our peak of the live lap timer) — see `resolve_lap_time_ms`; otherwise the measured time wins. `iLastTime` is 0 / a sentinel for a garage out-lap and goes **stale for the first lap or two out of a pit-lane race start** (holds the session-start→first-crossing span across several crossings). Adapter mints its own `lap_number` (`lap_counter`), so ACC lap numbers restart at 1 per recording. It also synthesises a `LapStarted` at every wrap (`pending_lap_started`) — ACC has no native lap-start signal, and without it a stint gap opened while idling in the garage keeps `stint_gap_during_lap` set and invalidates the first flying lap.
