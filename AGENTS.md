...
- ACC lap boundaries are driven by the `normalized_car_position` start/finish wrap, **not** solely `completed_lap` (which stalls across out-laps / return-to-garage). Out / in-laps are recorded tagged **invalid**; a wrap ACC never scored uses the peak live lap timer as the lap time. Adapter mints its own `lap_number` (`lap_counter`), so ACC lap numbers restart at 1 per recording.
